...
In a language like Alloy we would take advantage of a relational syntax to show that party a vs party b are swappable, particularly with respect to rules 2 and 4. We don't want to bind so tightly to A and B that we fail to match a situation where ~A~ is ~third~, and ~B~ is ~isfin~ or ~notfin~.
