...
In a language like Alloy we would take advantage of a relational syntax to show that party a vs party b are swappable, particularly with respect to rule 4. Because we don't want to bind so tightly to A and B that we fail to match a situation where ~A~ matches ~third~, and ~B~ matches ~isfin~ or ~notfin~.
