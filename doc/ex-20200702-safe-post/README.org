...
Question 2, "what is the impact of an Equity Financing on the Company's cap table?", gives us more information to work with:
...
For now, ignore the version of the function that has the little ~'~ on the end. More on that later.
...
To arrive at that number, we had to determine the number of conversion shares: 1,764,706.
...
We already know it should be 15%, but we reach that number in a bottom-up way.
...
But we will implement the operational logic using a constraint solver, because the Liquidity Capitalization definition is recursive.
...
*** Haskell: Linear Equations

We can phrase all of the above mathematics in the form of linear equations.

We try the ~linearEqSolver~ package.
