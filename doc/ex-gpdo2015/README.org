...
$\forall p:person, permitted(Thing = p) \iff child(p) \implies \lnot short(p)$

For all persons, permission is given by the following truth table:
...
$\forall p:person, permitted(Thing = p) \iff true \implies \lnot short(p)$

| true | short | permission |
|------+-------+------------|
| yes  | no    | yes        |
| yes  | yes   | no         |

There is no case in which "true" is "false", so the truth table only has two rows.
