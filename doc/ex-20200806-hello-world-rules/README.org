...
A number, a party and a list of actions make up a rule. Now let us see some terms and actions.
...
Let's put all this together in a GF abstract syntax.
...
Now we're able to generate a tree like this. (There's no concrete syntax yet -- exercise for the reader!)

![Abstract syntax tree for "Meng shan't plant a potato and shan't trade a potato"](https://listenmaa.fi/b/meng_shant_plant_nor_trade_potato.png)

It is very easy to transform the L4 code to this grammar. But as I said earlier, the final grammar shouldn't follow L4's syntax too faithfully.

Natural language has a different information structure, and we will likely want to use things like aggregation (_Meng shan't plant nor trade the potato_ instead of _Meng shan't plant the potato nor trade the potato_), anaphoric reference (_**it** is illegal if …_) etc.
