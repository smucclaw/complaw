...
With this grammar fragment, we can generate a tree like this. (There's no concrete syntax yet -- exercise for the reader!)
...
Natural language has a different information structure, and we will likely want to use things like aggregation (_Meng shan't plant nor trade the potato_ instead of _Meng shan't plant the potato and shan't trade the potato_), anaphoric reference (_it is illegal if …_) etc.
