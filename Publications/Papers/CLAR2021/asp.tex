...
\item[(A6)] $\forall r_{1},r_{2} \in R$ if $subject\_to(r_{1},r_{2}) \in M$, and $legally\_valid(r_{1},C_{r_{1}}) \in S$ and there exists a minimal conflicting set $i \in I$ such that $is\_legal(C_{r_{1}}) \in i$ and $i\setminus \{is\_legal(C_{r_{2}})\}\subseteq S $, then $legally\_valid(r_{2},C_{r_{2}}) \notin S$.
