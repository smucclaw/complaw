...
be presented in \secref{sec:intro_rule_modifiers}, and we will see how to give
...
\begin{proof}
  ...
  the rule order $\preceq_R$. Consider a rule $r_P \in {\cal F}_P$ with rule
  ...
  \item Assume that for arguments $(a_1, \dots, a_n)$, interpretation
    ...
    smaller than $r_P$ \wrt{} $\preceq_R$, by induction hypothesis, also the
...
\begin{proof} (Sketch)
  ...
  As in \lemmaref{lemma:mp_to_md}, the proof is by induction on $\preceq_R$.
