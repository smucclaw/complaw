...
$\textit{Proof}$ The proof proceeds almost exactly the same way as before. For
the inductive case we note that the following rule
\begin{verbatim}
createSub(subInst_r_c(W'),o+1) :- createSub(subInst_r_c(W),o+1), holds(p_z(h1,h2,..,hr)),
max_ab_lvl(M).    
\end{verbatim}

Creates the same new substitution instance of the rule $r_{c}$ and this then
causes the atom $query(p_z(h1,h2,..,hr),o+1)$ to get added to the program due
to proof search rules E9, E10. The base case $k=0$ is similarly dealt with
using proof search expansion rules E9, E8.
